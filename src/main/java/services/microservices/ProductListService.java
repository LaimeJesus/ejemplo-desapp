...
public class ProductListService extends GenericService<ProductList> {
	...
	throws 	ProductIsAlreadySelectedException, 
		...
		User exist = this.getUserService().authenticateUser(user);
		ProductList valid = this.getByUser(productList, exist);
		System.out.println("Antes de seleccionar: " + valid.getTotalAmount());
		valid.selectProduct(product, quantity);
		System.out.println("Despuess de seleccionar y antes de persistir: " + valid.getTotalAmount());
		this.getUserService().update(exist);
		ProductList valid2 = this.getByUser(productList, exist);
		System.out.println("Despues de persistir: " + valid2.getTotalAmount());
	...
	@Transactional
	public ProductList getByUser(ProductList example , User owner) throws UsernameOrPasswordInvalidException {
	...
	@Transactional
	public void applyOffer(User someUser, Offer someOffer, ProductList someProductList) throws UsernameOrPasswordInvalidException, MoneyCannotSubstractException {
